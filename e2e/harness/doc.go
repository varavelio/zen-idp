...
//
// A Harness compiles the zen-idp executable once per test run, writes an
// isolated YAML configuration into a per-test directory, binds a free
// loopback port, spawns the server with a dedicated state database, and
// waits until the discovery endpoint answers. Every test interacts with the
// instance exclusively through its HTTP surface with a browser-like cookie
// jar that never follows redirects, so tests can inspect every redirect
// target.
//
