...
//
// Creation is idempotent and records the first creation instant; checks
// report whether a gate is present; removal succeeds even when no gate
// exists, so recovery flows never fail on absent locks. IsLocked reports
// either gate and is the single check that login and session validation
// apply before granting access.
package lock
