...
// Create records a new active session for the authenticated identity and
// returns its browser credential token, formatted sess_{id}_{secret}. The
// token is the only value that can later authenticate the session; the
...
// Validate authenticates a browser token at the given instant and returns
// the authoritative session record it resolves to. The token must match the
// sess_{id}_{secret} format, its record must exist, its secret must match
// the stored digest, and the absolute expiration must not have passed.
